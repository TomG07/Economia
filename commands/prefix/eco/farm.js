            ...
            coletou.on('collect', async (i) => {
                ...
                if (i.customId === "wplant") {
                    ...
                    if (x.split("-")[0] === "batata") {
                        ...
                        await client.db.updateOne({ userId: `${i.guild.id}-${i.user.id}` }, { $inc: { "eco.coins": -valores, "eco.farm.seeds.batata.count": quantias, }, $set: { "eco.farm.seeds.batata.cooldown": Date.now() + 7200000, } });
                    } else if (x.split("-")[0] === "trigo") {
                        let valores = Number(`${x.split("-")[2]}`);
                        let quantias = Number(`${x.split("-")[1]}`);
                        await client.db.updateOne({ userId: `${i.guild.id}-${i.user.id}` }, { $inc: { "eco.coins": -valores, "eco.farm.seeds.trigo.count": quantias, }, $set: { "eco.farm.seeds.trigo.cooldown": Date.now() + 7200000, } });
                    } else if (x.split("-")[0] === "milho") {
                        let valores = Number(`${x.split("-")[2]}`);
                        let quantias = Number(`${x.split("-")[1]}`);
                        await client.db.updateOne({ userId: `${i.guild.id}-${i.user.id}` }, { $inc: { "eco.coins": -valores, "eco.farm.seeds.milho.count": quantias, }, $set: { "eco.farm.seeds.milho.cooldown": Date.now() + 7200000, } });
                    }
                }
            });
        });
